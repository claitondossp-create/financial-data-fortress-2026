# === FIX FORMAT STRING SCALER WITH NO SPACE ERROR ===
...
# ERRO REPARADO: O espaço literal DEVE ESTAR DENTRO DAS ASPAS do M.
# Antes: '"R$ "#,##0,, "Mi"' -> Espaço não parseado. Motor não multiplicava milhão, gerava " 606.000.000 Mi".
# Agora: '"R$ "#,##0,," Mi"' -> Espaço engolido pelas aspas; motor enxerga o vírgula-vírgula contínuo!
$correctFormat = '"R$ "#,##0,," Mi";-"R$ "#,##0,," Mi"'

foreach ($m in $tbl.Measures) {
    if ($targetMeasures -contains $m.Name) {
        $m.FormatString = $correctFormat
        Write-Host "FormatString Inteiro e Escalado para: $($m.Name)" -ForegroundColor Green
    }
}

$srv.Databases[0].Model.SaveChanges()
$srv.Disconnect()
Write-Host "FormatStrings de Milhões (R$ 297 Mi) perfeitamente injetados no TOM." -ForegroundColor Cyan
